...
(function () {
  ...
  // Human-readable labels for the detector's raw telemetry metrics -- used
  // to translate metric_scores (per-metric z-scores, real detector output)
  // into a plain-English "why is this risk score high" breakdown.
  const METRIC_LABEL = {
    network_traffic_mbps: 'Network anomaly',
    failed_logins: 'Failed logins',
    cpu_usage_pct: 'CPU load spike',
    data_egress_mb: 'Data egress',
    active_connections: 'Connection volume',
  };

  /**
   * Turn the detector's real per-metric z-scores into a "why is this score
   * high" breakdown, proportionally decomposing the *actual* risk_score
   * across whichever metrics deviated most -- e.g. z-scores [failed_logins:
   * 3.1, data_egress_mb: 2.4, ...] against risk_score 87 becomes
   * [{label:'Failed logins', points:47}, {label:'Data egress', points:36}, ...]
   * summing back to ~87. Nothing here is invented -- both the weights (z-
   * scores) and the total (risk_score) come straight from the detector.
   */
  function riskContributions(metricScores, riskScore, topN = 4) {
    if (!metricScores || typeof riskScore !== 'number') return [];
    const entries = Object.entries(metricScores).filter(([, z]) => z > 0);
    const totalZ = entries.reduce((sum, [, z]) => sum + z, 0);
    if (totalZ <= 0) return [];
    return entries
      .map(([key, z]) => ({
        key,
        label: METRIC_LABEL[key] || key.replace(/_/g, ' '),
        points: Math.round((z / totalZ) * riskScore),
        z,
      }))
  ...
  /** Compact "Label +N" chip row, for sector cards / alert rows. */
  function factorChipsHtml(contributions) {
    if (!contributions.length) return '';
    return contributions.map(c =>
      `<span class="factor-chip"><span class="factor-chip-label">${c.label}</span><span class="factor-chip-pts">+${c.points}</span></span>`
    ).join('');
  }

  /** Full bar-style breakdown, for the drill-down modal / incident detail. */
  function factorBarsHtml(contributions) {
    if (!contributions.length) {
      return '<div style="color:var(--text-dim); font-size:12px;">No per-metric breakdown available for this alert.</div>';
    }
    const max = Math.max(...contributions.map(c => c.points), 1);
    return contributions.map(c => `
      <div class="factor-bar-row">
        <span class="factor-bar-label">${c.label}</span>
        <div class="factor-bar-track"><div class="factor-bar-fill" style="width:${Math.max(6, (c.points / max) * 100)}%"></div></div>
        <span class="factor-bar-pts">+${c.points}</span>
      </div>`).join('');
  }
  ...
  // ---------- shared modal shell (drill-down alert detail, incident detail) ----------
  // Reuses the #drill-down-modal / .drill-down-card markup+CSS that already
  // ships in style.css (originally scaffolded for the analyst log drill-down
  // but never wired up), rather than inventing a parallel modal system.
  function ensureModal() {
    ...
    modal.innerHTML = `
      <div class="drill-down-card" role="dialog" aria-modal="true">
        <button class="drill-down-close" id="kavach-modal-close" aria-label="Close">&times;</button>
        <div id="kavach-modal-body"></div>
      </div>`;
    document.body.appendChild(modal);
    modal.addEventListener('click', (e) => { if (e.target === modal) closeModal(); });
    modal.querySelector('#kavach-modal-close').addEventListener('click', closeModal);
    document.addEventListener('keydown', (e) => { if (e.key === 'Escape') closeModal(); });
    return modal;
  }
  function openModal(html) {
    const modal = ensureModal();
    modal.querySelector('#kavach-modal-body').innerHTML = html;
  ...
  window.Kavach = {
    ...
    METRIC_LABEL, riskContributions, factorChipsHtml, factorBarsHtml,
